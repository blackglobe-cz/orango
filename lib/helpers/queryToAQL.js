...
class QueryToAQL {
  ...
  async linkCollection(targets, key){

    if (isObject(key)) {
      for (let name in key) {
        let modelName = capitalize(name)

        if (targets.indexOf(modelName) !== -1) {
          let fromModelCls = this.orango.model(modelName)
          return fromModelCls.collectionName + '/' + key[name]
        }
      }
  ...
  async parseLink(query) {
    ...
    if (!query.data.targets.from) {
      throw new Error('Missing required "from" for link')
    }

    if (!query.data.targets.to) {
      throw new Error('Missing required "to" for link')
    }

    query.data._to = this.linkCollection(toTargets, query.data.targets.to)
    query.data._from = this.linkCollection(fromTargets, query.data.targets.from)
